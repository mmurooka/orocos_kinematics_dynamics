...
int main()
{
  ...
  // Joint ///////////////////
  const Joint *j1;
  for(std::vector<Segment>::iterator it = chain1.segments.begin(); it != chain1.segments.end(); it++) {
    if (it->getJoint().getName() == "Joint 2") {
      j1 = &(it->getJoint());
      break;
    }
  }
  std::cout << "=== Segment ===" << std::endl;
  std::cout << "Name: " << j1->getName() <<
    "  max: " << j1->JointUpper() << "  min: " << j1->JointLower() << std::endl;
